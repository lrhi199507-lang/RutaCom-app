...
import { 
  ...
  X, CheckCircle, Repeat, ArrowLeftRight, Settings, Info, Check, Star, Navigation, Archive, AlertTriangle
} from 'lucide-react';
import MapaView from '../Map/MapaView'; 

// 🔥 BLINDAJE 1: Funciones de formato seguras
...
// 🔥 BLINDAJE 2: Validación estricta para el MapaView
const validarCoordenadas = (coords) => {
  return coords && typeof coords === 'object' && ('lat' in coords || 'latitude' in coords);
};
...
const ViajeCardChofer = ({ viaje, onEdit, onArchivar, onClickGestionar }) => {
  try {
    ...
    if (fechaViaje && typeof fechaViaje === 'string' && horaViaje && typeof horaViaje === 'string' && !estaEnCurso && !esFinalizado && !esCancelado && pasajerosCount === 0) {
      ...
      if (new Date() > limiteTiempo) esVencido = true;
    }
    ...
    const mapaSeguro = validarCoordenadas(viaje.coordsOrigen) && validarCoordenadas(viaje.coordsDestino);
    ...
    return (
      <div className={`bg-white p-6 rounded-[30px] border shadow-sm ${esRetorno ? 'border-dashed border-emerald-200 bg-emerald-50/10' : 'border-slate-100'} relative space-y-4`}>
        ...
        <div className={`h-32 rounded-2xl overflow-hidden mb-2 relative pointer-events-none bg-slate-100 ${(esFinalizado || esCancelado) ? 'opacity-60 grayscale-[50%]' : ''}`}>
          ...
          {mapaSeguro ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center"><MapPin size={24} className="text-slate-300" /></div>
          )}
        </div>
        ...
        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 grid grid-cols-2 gap-4">
          <div className="flex items-center gap-2.5"><Calendar size={16} className="text-blue-500"/><p className="text-xs font-bold text-slate-700 capitalize">{formatearFechaCorta(fechaStr)}</p></div>
          <div className="flex items-center gap-2.5"><Clock size={16} className="text-blue-500"/><p className="text-xs font-bold text-slate-700">{formatearHora12h(horaStr)}</p></div>
    ...
    return <div className="bg-slate-100 p-6 rounded-3xl border border-slate-200 text-center text-slate-400"><AlertTriangle className="mx-auto mb-2 opacity-50"/>Viaje con formato antiguo</div>;
  }
};
  ...
  try {
    ...
    return (
      <div className={`bg-white p-6 rounded-[30px] shadow-sm border space-y-4 relative overflow-hidden ${esConfirmado && esActivo ? 'border-blue-200' : 'border-slate-100'}`}>
        <div className={`absolute inset-0 pointer-events-none opacity-40 z-0 bg-slate-50 ${(!esActivo) ? 'grayscale-[50%]' : ''}`}>
           {mapaSeguro && <MapaView origen={viaje.coordsOrigen} destino={viaje.coordsDestino} interactivo={false} />}
        ...
        <div className="bg-slate-50/90 backdrop-blur-md p-4 rounded-2xl border border-slate-100 flex justify-between gap-4 relative z-10">
          <div className="flex items-center gap-2"><Calendar size={16} className="text-blue-500 shrink-0"/><p className="text-xs font-bold text-slate-700 capitalize">{formatearFechaCorta(fechaStr)}</p></div>
          <div className="flex items-center gap-2"><Clock size={16} className="text-blue-500 shrink-0"/><p className="text-xs font-bold text-slate-700">{formatearHora12h(horaStr)}</p></div>
        </div>
    ...
    return <div className="bg-slate-100 p-6 rounded-3xl border border-slate-200 text-center text-slate-400"><AlertTriangle className="mx-auto mb-2 opacity-50"/>Error al procesar tarjeta</div>;
  }
};
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <div className="p-4 pt-8 bg-white border-b border-slate-100 sticky top-0 z-30">
        ...
        <div className="flex bg-slate-100 p-1.5 rounded-full relative shadow-inner">
          ...
          <button onClick={() => setActiveTab('chofer')} className={`relative flex-1 py-3.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all z-10 flex items-center justify-center gap-2 ${activeTab === 'chofer' ? 'text-white' : 'text-slate-500 hover:text-slate-700'}`}>
            Como Chofer
          </button>
          <button onClick={() => setActiveTab('pasajero')} className={`relative flex-1 py-3.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all z-10 flex items-center justify-center gap-2 ${activeTab === 'pasajero' ? 'text-white' : 'text-slate-500 hover:text-slate-700'}`}>
            Como Pasajero
          </button>
        </div>
      </div>
        ...
        {activeTab === 'chofer' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            {subTabChofer === 'activos' ? (
              viajesChoferActivos.length > 0 ? (
                <div className="space-y-4">{viajesChoferActivos.map(viaje => <ViajeCardChofer key={viaje.id} viaje={viaje} onEdit={() => setEditingViaje(viaje)} onArchivar={handleArchivarViaje} onClickGestionar={onVerDetalles} />)}</div>
              ) : (
              ...
              viajesChoferHistorial.length > 0 ? (
                <div className="space-y-4 opacity-90">{viajesChoferHistorial.map(viaje => <ViajeCardChofer key={viaje.id} viaje={viaje} onEdit={() => {}} onArchivar={() => {}} onClickGestionar={onVerDetalles} />)}</div>
              ) : (
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            {subTabPasajero === 'activos' ? (
              pasajerosActivosTodos.length > 0 ? (
                <div className="space-y-4">{pasajerosActivosTodos.map(viaje => <ViajeCardPasajero key={viaje.id} viaje={viaje} userData={userData} onClickGestionar={onVerDetalles} />)}</div>
              ) : (
              ...
              pasajerosHistorialTodos.length > 0 ? (
                <div className="space-y-4 opacity-90">{pasajerosHistorialTodos.map(viaje => <ViajeCardPasajero key={viaje.id} viaje={viaje} userData={userData} onClickGestionar={onVerDetalles} />)}</div>
              ) : (
