...
export const VistaMisViajes = ({ 
  ...
  const [subVista, setSubVista] = useState("pasajero"); // "pasajero" o "chofer"

  // 1. LÓGICA DE FILTRADO
  // Como pasajero: Viajes donde mi ID está en la lista de pasajeros o en espera
  const viajesComoPasajero = viajes.filter(v => 
    v.pasajeros?.some(p => p.id === userData?.id) || 
    v.reservasPendientes?.some(p => p.id === userData?.id)
  );

  // Como chofer: Viajes que yo publiqué
  const misPublicaciones = viajes.filter(v => v.conductorId === userData?.id);

  // Separar historial (viajes de fechas pasadas o completados)
  const hoy = new Date().toISOString().split('T')[0];
  const historial = viajes.filter(v => {
    const fechaV = v.fecha ? v.fecha.split('T')[0] : "";
    return fechaV < hoy && (v.conductorId === userData?.id || v.pasajeros?.some(p => p.id === userData?.id));
  });

  return (
    <div className="min-h-screen bg-slate-50 pb-28">
      {/* HEADER CON TABS ESTILO PERFIL */}
              ...
              {historial.map(v => (
                <div key={v.id} className="bg-white/50 p-4 rounded-[25px] border border-dashed border-slate-200 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase">{v.fecha}</p>
                  </div>
...
const CardChofer = ({ viaje, onEdit, onDelete, onAceptar, onRechazar }) => (
  <div className="bg-white p-5 rounded-[32px] border border-slate-100 shadow-sm space-y-4">
    <div className="flex justify-between items-start">
      <div>
        <span className="bg-blue-50 text-blue-600 text-[8px] font-black px-2 py-1 rounded-lg uppercase italic mb-2 inline-block">Tu Publicación</span>
        <h4 className="text-sm font-black italic text-slate-800 uppercase">{viaje.origen?.split(',')[0]} → {viaje.destino?.split(',')[0]}</h4>
        <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">{viaje.fecha} • {viaje.horaSalida || viaje.hora}</p>
      </div>
      <div className="flex gap-2">
        <button onClick={() => onEdit(viaje)} className="p-3 bg-slate-50 text-slate-400 rounded-2xl active:text-blue-600 active:bg-blue-50"><Edit3 size={16}/></button>
        <button onClick={() => onDelete(viaje.id)} className="p-3 bg-slate-50 text-slate-400 rounded-2xl active:text-red-600 active:bg-red-50"><Trash2 size={16}/></button>
      </div>
    </div>
        ...
        {viaje.reservasPendientes.map(p => (
          <div key={p.id} className="flex items-center justify-between bg-white p-3 rounded-xl shadow-sm">
            ...
            <div className="flex gap-1">
              <button onClick={() => onAceptar(viaje.id, p)} className="p-2 bg-green-500 text-white rounded-lg shadow-md"><CheckCircle2 size={14}/></button>
              <button onClick={() => onRechazar(viaje.id, p)} className="p-2 bg-slate-100 text-slate-400 rounded-lg"><X size={14}/></button>
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="bg-white p-5 rounded-[32px] border border-slate-100 shadow-sm">
      <div className="flex justify-between items-center mb-3">
        ...
        <span className={`text-[8px] font-black uppercase px-2 py-1 rounded-lg italic ${isConfirmado ? 'bg-green-100 text-green-600' : 'bg-amber-100 text-amber-600'}`}>
          {isConfirmado ? 'Confirmado' : 'Esperando'}
        </span>
      </div>
      <div className="flex items-center gap-4 text-[9px] font-bold text-slate-400 uppercase">
        <span className="flex items-center gap-1"><Calendar size={12}/> {viaje.fecha}</span>
        <span className="flex items-center gap-1"><Car size={12}/> {viaje.conductor}</span>
      </div>
    </div>
  );
};
...
const EmptyState = ({ msg }) => (
  <div className="text-center py-12 bg-white rounded-[35px] border border-dashed border-slate-200">
    <p className="text-slate-400 font-black italic uppercase text-[10px] tracking-widest">{msg}</p>
  </div>
);
