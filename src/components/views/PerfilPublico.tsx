...
const PerfilPublico = ({ conductor, onClose }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[500] bg-white flex flex-col animate-in slide-in-from-right duration-300">
      ...
      {/* CABECERA BLANCA (Mismo estilo que el resto de la app) */}
      <div className="bg-white px-6 pt-12 pb-4 flex items-center justify-between border-b border-slate-50">
        <div className="flex items-center gap-3">
          {/* Avatar estilo "D" azul */}
          ...
          <div>
            <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Modo Pasajero</p>
            <p className="text-sm font-black text-slate-800 leading-none">{conductor.nombre}</p>
          </div>
        </div>

        {/* Botón Volver con estilo de Wallet/Píldora */}
        <button 
          onClick={onClose}
          className="bg-slate-900 text-white px-5 py-3 rounded-full flex items-center gap-2 shadow-lg active:scale-95 transition-all"
        >
      ...
      <div className="flex-1 overflow-y-auto px-6 pt-8">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="w-28 h-28 bg-white rounded-[35px] border-4 border-slate-50 shadow-2xl overflow-hidden mb-4 flex items-center justify-center relative">
        ...
        <div className="space-y-1">
          <p className="text-[9px] font-black text-blue-500 uppercase tracking-[3px] ml-2 italic">Sobre el conductor</p>
          <div className="bg-slate-50 p-6 rounded-[35px] border border-slate-100/50">
        ...
        {/* ESTILO DE VIAJE */}
        <div className="mt-8">
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-[3px] mb-4 ml-2 italic">Estilo de viaje</p>
          <div className="grid grid-cols-2 gap-3">
            <div className={`p-4 rounded-[25px] border-2 flex flex-col items-center gap-2 transition-all ${conductor.hablador ? 'bg-blue-50/50 border-blue-600' : 'bg-slate-50 border-transparent opacity-50'}`}>
              <MessageSquare size={20} className={conductor.hablador ? 'text-blue-600' : 'text-slate-400'} />
              <p className="text-[9px] font-black uppercase text-slate-800">{conductor.hablador ? 'Conversador' : 'Tranquilo'}</p>
            </div>
            <div className={`p-4 rounded-[25px] border-2 flex flex-col items-center gap-2 transition-all ${conductor.musica ? 'bg-blue-50/50 border-blue-600' : 'bg-slate-50 border-transparent opacity-50'}`}>
              <Music size={20} className={conductor.musica ? 'text-blue-600' : 'text-slate-400'} />
        ...
        {/* OPINIONES */}
        <div className="mt-8">
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-[3px] mb-4 ml-2 italic">Opiniones y reseñas</p>
          <button className="w-full bg-slate-50 border border-slate-100 p-5 rounded-[35px] flex items-center justify-between active:scale-95 transition-all">
        ...
        {/* ESTADÍSTICAS */}
        <div className="mt-8 mb-32 grid grid-cols-2 gap-4">
            <div className="flex items-center gap-3 bg-blue-50/30 p-4 rounded-[25px]">
              <ShieldCheck size={18} className="text-blue-500" />
              <p className="text-[8px] font-black text-slate-600 uppercase leading-tight italic">Identidad<br/>Verificada</p>
            </div>
            <div className="flex items-center gap-3 bg-slate-50 p-4 rounded-[25px]">
