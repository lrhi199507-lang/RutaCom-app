    ...
    <div className="fixed inset-0 z-[500] bg-[#1F2937]/60 backdrop-blur-sm flex items-end justify-center animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-md rounded-t-[40px] p-6 pb-10 animate-in slide-in-from-bottom max-h-[85vh] overflow-y-auto no-scrollbar">
        ...
        <div className="space-y-4">
          <div className="bg-[#063971]/5 border border-[#063971]/10 p-5 rounded-[25px] flex gap-4">
          ...
          <div className="bg-[#10B981]/10 border border-[#10B981]/20 p-5 rounded-[25px] flex gap-4">
            <div className="mt-1"><ShieldCheck className="text-[#10B981]" size={24} /></div>
            <div>
              <h3 className="font-black uppercase text-[11px] text-[#10B981] tracking-widest mb-1">Costos compartidos</h3>
              <p className="text-xs text-[#1F2937] font-medium leading-relaxed">El precio no es el de un viaje privado. Pagas solo por tu asiento para ayudar al conductor a cubrir los gastos de la ruta (gasolina, peajes, etc).</p>
            </div>
          </div>

          <div className="bg-amber-50 border border-amber-100 p-5 rounded-[25px] flex gap-4">
            <div className="mt-1"><Clock className="text-amber-600" size={24} /></div>
            <div>
              <h3 className="font-black uppercase text-[11px] text-amber-700 tracking-widest mb-1">Puntualidad y Respeto</h3>
              <p className="text-xs text-[#1F2937] font-medium leading-relaxed">Al ser un viaje compartido, el tiempo de todos vale. Llega a la hora acordada al punto de encuentro para no retrasar a los demás pasajeros.</p>
            </div>
          </div>
        </div>

        <button onClick={onClose} className="w-full bg-[#063971] text-white rounded-full p-4 font-black uppercase text-xs mt-6 active:scale-95 transition-all shadow-lg shadow-[#063971]/30">
          ¡Entendido!
        </button>
      </div>
    </div>
    ...
    <div className="bg-slate-50 min-h-screen flex flex-col font-sans relative">
      ...
      <div className="p-4 bg-white/90 backdrop-blur-md sticky top-0 z-10 border-b border-slate-100">
        <div className="flex bg-slate-100 p-1.5 rounded-[22px] max-w-md mx-auto shadow-inner">
          <button onClick={() => setPestañaActiva('publico')} className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase transition-all ${view === 'publico' ? 'bg-white text-[#063971] shadow-sm' : 'text-slate-400'}`}>Mi Perfil</button>
          <button onClick={() => setPestañaActiva('cuenta')} className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase transition-all ${view === 'cuenta' ? 'bg-white text-[#063971] shadow-sm' : 'text-slate-400'}`}>Mi Cuenta</button>
        </div>
      </div>
        ...
        {view === 'publico' && (
          <div className="p-5 space-y-6 animate-in fade-in duration-500">
            {auth.currentUser && !auth.currentUser.emailVerified && (
              <div className="bg-[#063971] rounded-[30px] p-5 shadow-lg border-b-4 border-[#031E3F]">
            ...
            <div className="bg-white p-6 rounded-[35px] shadow-sm border border-slate-100 space-y-4">
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1"> Próximo Nivel: {faltan > 0 ? rangoDatos.meta : 'MÁXIMO'} VJS </p> <p className="text-sm font-black text-[#1F2937] italic uppercase">
                  {faltan > 0 ? `Te faltan ${faltan} viajes` : "¡Eres Leyenda!"}
                  </p>
                 </div>
                <div className="text-[10px] font-black text-[#063971] bg-[#063971]/10 px-3 py-1 rounded-full"> {totalTrayectoria} / {rangoDatos.meta} VJS </div>
              </div>
              <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-[#063971] to-[#10B981] transition-all duration-1000" style={{ width: `${porcentajeNivel}%` }} />
              </div>
            </div>
            ...
            <div className="bg-white p-7 rounded-[40px] shadow-sm border border-slate-100">
              <div className="flex justify-between items-center mb-5">
                <div>
                  ...
                  <p className="text-lg font-black text-[#1F2937] uppercase">Verificado al <span className="text-[#10B981] italic">{porcentajeConfianza.toFixed(0)}%</span></p>
                </div>
                <div className="bg-[#10B981]/10 px-4 py-2 rounded-2xl text-2xl font-black text-[#10B981]">{porcentajeConfianza.toFixed(0)}%</div>
              </div>
              <div className="w-full h-4 bg-slate-100 rounded-full p-1 shadow-inner">
                <div className="h-full bg-gradient-to-r from-[#10B981] to-emerald-400 rounded-full transition-all duration-1000" style={{ width: `${porcentajeConfianza}%` }} />
              </div>
            </div>
          </div>
        )}
          ...
          <div className="p-5 space-y-8 animate-in slide-in-from-right duration-500 pb-24">
            ...
            <div className="bg-white p-5 rounded-[30px] shadow-sm mt-4 border border-slate-100">
              ...
              <div className="flex flex-row gap-3">
                <div onClick={() => togglePreferencia('hablador', !(userData?.hablador || false))} className={`flex-1 py-3 rounded-2xl border flex flex-col items-center cursor-pointer transition-colors ${(userData?.hablador || false) ? 'bg-[#063971] border-[#063971]' : 'bg-slate-50 border-slate-100'}`}>
                ...
                <div onClick={() => togglePreferencia('musica', !(userData?.musica || false))} className={`flex-1 py-3 rounded-2xl border flex flex-col items-center cursor-pointer transition-colors ${(userData?.musica || false) ? 'bg-[#063971] border-[#063971]' : 'bg-slate-50 border-slate-100'}`}>
      ...
      {modalVisible && (
        <div className="fixed inset-0 z-[200] flex items-end justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-md rounded-[40px] p-10 animate-in slide-in-from-bottom">
            <h4 className="text-[10px] font-black text-[#063971] uppercase mb-6 italic tracking-widest text-center">Editar {tipoEdicion?.label}</h4>
            ...
            {tipoEdicion?.id === 'bio' ? (
              <textarea value={nuevoValor} onChange={(e) => setNuevoValor(e.target.value)} className="w-full bg-slate-50 p-6 rounded-3xl font-medium text-sm mb-8 outline-none border-2 border-slate-100 focus:border-[#063971]/30 min-h-[150px] resize-none text-[#1F2937] transition-colors" placeholder="Ej: Hola..." autoFocus />
            ) : (
              <input value={nuevoValor} onChange={(e) => setNuevoValor(e.target.value)} className="w-full bg-slate-50 p-5 rounded-2xl font-black text-lg mb-8 outline-none border-2 border-slate-100 focus:border-[#063971]/30 uppercase text-center text-[#1F2937] transition-colors" autoFocus />
            )}
            <button disabled={cargando} onClick={() => guardarCambios()} className="w-full bg-[#063971] text-white p-5 rounded-[25px] disabled:opacity-50 font-black uppercase text-xs shadow-lg shadow-[#063971]/30 active:scale-95 transition-all">Guardar Cambios</button>
          </div>
        </div>
      )}
      ...
      {confirmacionCedula && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#1F2937]/80 backdrop-blur-sm" onClick={() => !cargando && setConfirmacionCedula(false)} />
          <div className="relative bg-white w-full max-w-sm rounded-[40px] p-8 text-center animate-in zoom-in-95">
            ...
            <h3 className="text-lg font-black text-[#1F2937] uppercase italic mb-2">Confirmación de Cédula</h3>
            <p className="text-xs font-bold text-slate-500 mb-6 leading-relaxed">¿Estás seguro de que tu documento es <span className="text-[#1F2937] font-black text-sm">{nuevoValor}</span>?</p>
            <div className="flex gap-3">
              <button disabled={cargando} onClick={() => setConfirmacionCedula(false)} className="flex-1 bg-slate-100 text-slate-500 hover:bg-slate-200 font-black uppercase text-[10px] py-4 rounded-2xl transition-colors">Corregir</button>
              <button disabled={cargando} onClick={() => guardarCambios(true)} className="flex-[1.5] bg-orange-500 text-white font-black uppercase text-[10px] py-4 rounded-2xl shadow-lg active:scale-95 transition-all">Sí, Confirmar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {pasoDocumento.activa && (
        <div className="fixed inset-0 z-[300] bg-[#1F2937]/95 backdrop-blur-sm flex flex-col p-6 items-center justify-center space-y-6 animate-in fade-in duration-200">
          {!fotoDocTemporal ? (
            <div className="bg-white p-8 rounded-[40px] border border-slate-100 w-full max-w-sm text-center shadow-2xl">
              <div className="w-20 h-20 bg-[#063971]/10 rounded-full flex items-center justify-center mx-auto mb-6 border-4 border-[#063971]/20"><Camera size={35} className="text-[#063971]" /></div>
              <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Subir documento</p>
              <h3 className="text-xl font-black italic text-[#1F2937] uppercase mb-8">Adjuntar Imagen</h3>
              <div className="space-y-3">
                <button onClick={() => capturarDocumento(CameraSource.Camera)} className="w-full bg-[#063971] text-white rounded-2xl py-4 flex items-center justify-center gap-3 shadow-lg shadow-[#063971]/30 active:scale-95 transition-all"><Camera size={18} /><span className="font-black text-[11px] uppercase tracking-widest">Abrir Cámara</span></button>
                <button onClick={() => capturarDocumento(CameraSource.Photos)} className="w-full bg-slate-50 text-[#063971] border border-[#063971]/30 rounded-2xl py-4 flex items-center justify-center gap-3 hover:bg-[#063971]/5 active:scale-95 transition-all"><ImageIcon size={18} /><span className="font-black text-[11px] uppercase tracking-widest">Abrir Galería</span></button>
              </div>
              <button onClick={() => setPasoDocumento({...pasoDocumento, activa: false})} className="text-slate-400 hover:text-[#1F2937] font-black uppercase text-[10px] mt-8 tracking-widest transition-colors">Cancelar</button>
            </div>
          ) : (
            <div className="w-full max-w-sm space-y-4 text-center bg-white p-6 rounded-[40px] shadow-2xl">
              <div className="w-full aspect-video rounded-3xl overflow-hidden border-4 border-[#063971]/20 shadow-inner"><img src={fotoDocTemporal} className="w-full h-full object-cover" /></div>
              <button disabled={cargando} onClick={subirDocumentoFinal} className="w-full disabled:opacity-50 bg-[#10B981] text-white py-5 rounded-[25px] font-black uppercase text-xs shadow-xl shadow-[#10B981]/30 active:scale-95 transition-all">{cargando ? "SUBIENDO..." : "Enviar Documento"}</button>
              <button disabled={cargando} onClick={() => setFotoDocTemporal(null)} className="w-full text-slate-400 hover:text-[#1F2937] font-black uppercase text-[10px] py-3 transition-colors">Elegir otra imagen</button>
            </div>
          )}
        </div>
      )}
      ...
      {modalClave && (
        <div className="fixed inset-0 bg-[#1F2937]/90 backdrop-blur-sm z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-[35px] border border-slate-100 p-8 shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#063971]/10 rounded-full blur-3xl pointer-events-none"></div>
      ...
      {pasoFoto && (
        <div className="fixed inset-0 z-[200] bg-white flex flex-col p-8 items-center justify-center text-center animate-in fade-in">
          {!fotoTemporal ? (
            <>
              <div className="w-44 h-44 bg-slate-50 rounded-full flex items-center justify-center mb-10 border-4 border-slate-100 shadow-inner"><User size={80} className="text-slate-300" /></div>
              <div className="w-full space-y-3">
                <button onClick={() => seleccionarImagen(CameraSource.Camera)} className="w-full bg-[#063971] text-white p-5 rounded-[25px] font-black uppercase text-xs flex items-center justify-center gap-3 shadow-lg shadow-[#063971]/30 active:scale-95 transition-all"><Camera size={18} /> Tomar Foto Ahora</button>
                <button onClick={() => seleccionarImagen(CameraSource.Photos)} className="w-full bg-slate-100 text-[#063971] p-5 rounded-[25px] font-black uppercase text-xs flex items-center justify-center gap-3 active:scale-95 transition-all hover:bg-slate-200"><FileText size={18} className="text-[#063971]" /> Elegir de la Galería</button>
              </div>
              <button onClick={() => setPasoFoto(false)} className="text-slate-400 font-black uppercase text-[10px] mt-8 tracking-widest hover:text-[#1F2937] transition-colors">Tal vez luego</button>
            </>
          ) : (
            <>
              <div className="w-64 h-64 rounded-full overflow-hidden border-8 border-[#063971]/10 mb-10 shadow-2xl"><img src={fotoTemporal} className="w-full h-full object-cover" /></div>
              <button disabled={cargando} onClick={subirFotoConfirmada} className="w-full bg-[#10B981] disabled:opacity-50 text-white p-6 rounded-[25px] font-black uppercase text-xs shadow-xl shadow-[#10B981]/30 active:scale-95 transition-all">{cargando ? "SUBIENDO..." : "Confirmar Foto"}</button>
              <button disabled={cargando} onClick={() => setFotoTemporal(null)} className="text-slate-400 font-black uppercase text-[10px] mt-6 tracking-widest hover:text-[#1F2937] transition-colors">Elegir otra</button>
            </>
          )}
        </div>
      )}
