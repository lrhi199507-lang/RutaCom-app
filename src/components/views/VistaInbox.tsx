  ...
  return (
    <div 
      ...
      className={`bg-white p-4 rounded-[25px] border transition-all active:scale-95 flex items-center gap-4 cursor-pointer
        ${isUnread ? 'border-[#063971]/30 shadow-md bg-[#063971]/5' : 'border-slate-100 shadow-sm hover:border-[#063971]/20'}`}
    >
      <div className="relative flex-shrink-0">
        ...
        {/* PUNTO DE NO LEÍDO: Azul Tránsito */}
        {isUnread && <div className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-[#063971] border-2 border-white rounded-full animate-pulse" />}
      </div>

      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-start mb-1">
          <h4 className={`text-sm truncate pr-2 uppercase ${isUnread ? 'font-black text-[#1F2937]' : 'font-bold text-[#1F2937]/80'}`}>
            {nombreContacto || 'Usuario Dame la cola'}
          </h4>
          <span className={`text-[9px] font-bold whitespace-nowrap mt-0.5 ${isUnread ? 'text-[#063971]' : 'text-slate-400'}`}>
            {horaFormateada}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 mb-1.5 overflow-hidden">
          {soyConductor ? (
            // BADGE CONDUCTOR: Verde Éxito
            <div className="flex items-center gap-1 bg-[#10B981]/10 border border-[#10B981]/20 px-2 py-0.5 rounded-full flex-shrink-0">
              <Car size={10} className="text-[#10B981]" />
              <span className="text-[8px] font-black text-[#10B981] uppercase tracking-wider">Tú conduces</span>
            </div>
          ) : (
            // BADGE PASAJERO: Azul Tránsito
            <div className="flex items-center gap-1 bg-[#063971]/5 border border-[#063971]/20 px-2 py-0.5 rounded-full flex-shrink-0">
              <User size={10} className="text-[#063971]" />
              <span className="text-[8px] font-black text-[#063971] uppercase tracking-wider">Eres pasajero</span>
            </div>
          )}
        ...
        <p className={`text-xs truncate ${isUnread ? 'font-bold text-[#1F2937]' : 'font-medium text-slate-500'}`}>
...
export const VistaInbox = ({ 
  ...
  if (!userData?.id) return (
    <div className="h-screen flex flex-col items-center justify-center font-black text-[#063971] italic uppercase bg-white">
      <MessageCircle size={40} className='mb-2 animate-pulse text-[#063971]'/>
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans">
      <div className="p-6 pt-10 bg-white">
        <h2 className="text-3xl font-extrabold text-[#1F2937] tracking-tighter">Mensajes</h2>
