...
export const PerfilUsuarioDetalle = ({ uid, onClose }) => {
  ...
  const obtenerRango = (totalViajes) => {
    ...
    return { nombre: "NOVATO", color: "bg-[#063971]" }; // Azul Tránsito para Novato
  };

  if (cargando) {
    return (
      <div className="fixed inset-0 z-[200] bg-slate-50 flex flex-col items-center justify-center animate-in fade-in">
        <div className="w-12 h-12 border-4 border-[#063971]/20 border-t-[#063971] rounded-full animate-spin mb-4"></div>
        <p className="text-xs font-black text-[#063971]/50 uppercase tracking-[3px] animate-pulse">Cargando Perfil...</p>
      </div>
    );
  }
  ...
  if (!usuario) {
    return (
      <div className="fixed inset-0 z-[200] bg-slate-50 flex flex-col items-center justify-center p-6">
        ...
        <button onClick={onClose} className="mt-6 bg-[#063971] text-white px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg">Volver</button>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-slate-50 flex flex-col font-sans animate-in slide-in-from-bottom duration-300">
      ...
      <div className="p-4 pt-6 flex justify-between items-center sticky top-0 bg-slate-50/90 backdrop-blur-md z-10 border-b border-slate-100">
        <button onClick={onClose} className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm border border-slate-100 text-slate-500 hover:text-[#063971] active:scale-90 transition-all">
          <ArrowLeft size={18} />
        </button>
        <p className="text-[10px] font-black text-[#1F2937] uppercase tracking-[3px]">Perfil Público</p>
      ...
      <div className="flex-1 overflow-y-auto p-5 pb-32 space-y-4">
        ...
        <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 flex flex-col items-center relative mt-4">
          ...
          <div className="w-28 h-28 bg-[#063971]/5 rounded-[35px] border-4 border-white shadow-xl overflow-hidden mb-5 flex items-center justify-center mt-2">
            {usuario.fotoPerfil ? (
              <img src={usuario.fotoPerfil} className="w-full h-full object-cover" alt="Perfil" />
            ) : (
              <span className="text-[#063971]/40 font-black italic text-4xl">{usuario.nombre?.charAt(0) || "U"}</span>
            )}
          </div>
          
          <div className="flex items-center gap-2 mb-1">
            <h2 className="text-2xl font-black text-[#1F2937] uppercase italic tracking-tighter text-center">{usuario.nombre || "Usuario"}</h2>
          ...
          <div className="flex w-full justify-center gap-8 mt-6 border-t border-slate-50 pt-5">
            <div className="text-center">
              ...
              <p className="font-black text-[#063971] italic text-lg leading-none">{viajesCond} <span className="text-[10px]">VJS</span></p>
            </div>
        ...
        {/* BIO Y PREFERENCIAS */}
        {usuario.bio && (
          <div className="bg-[#063971]/5 p-6 rounded-[30px] border border-[#063971]/10 relative">
            <Quote size={24} className="text-[#063971]/20 absolute top-4 right-4 rotate-180" />
            <p className="text-[10px] font-black text-[#063971] uppercase tracking-widest mb-2">Sobre Mí</p>
            <p className="text-xs font-bold text-[#1F2937] leading-relaxed italic">"{usuario.bio}"</p>
          </div>
        )}
        ...
        <button 
          ...
          className={`w-full bg-white border border-slate-100 p-5 rounded-[30px] flex items-center justify-between shadow-sm transition-all group ${estadisticas.total > 0 ? 'hover:bg-[#063971]/5 active:scale-95 cursor-pointer' : 'cursor-default opacity-80'}`}
        >
          <div className="flex items-center gap-4">
            ...
            <div className="text-left">
              ...
              <p className="text-lg font-black text-[#1F2937] italic leading-none">
          ...
          {estadisticas.total > 0 && <ArrowLeft size={18} className="rotate-180 text-[#063971] group-hover:translate-x-1 transition-transform" />}
        </button>
      ...
      {mostrarResenas && (
        <div className="fixed inset-0 z-[300] bg-white flex flex-col animate-in slide-in-from-bottom duration-300">
          <div className="p-4 pt-6 border-b border-slate-50 flex items-center justify-between bg-white sticky top-0 z-10 shadow-sm">
            <button onClick={() => setMostrarResenas(false)} className="w-10 h-10 bg-slate-50 rounded-full text-slate-500 hover:text-[#063971] active:scale-90 transition-all flex items-center justify-center">
              <ArrowLeft size={18} />
            </button>
            <h3 className="font-black italic uppercase text-[#1F2937] text-sm tracking-widest">Opiniones ({estadisticas.total})</h3>
            <div className="w-10"></div>
          </div>
              ...
              <div key={resena.id} className="bg-white p-5 rounded-[25px] border border-slate-100 shadow-sm">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-[#063971]/5 rounded-[12px] flex items-center justify-center border border-[#063971]/10">
                      <User size={16} className="text-[#063971]" />
                    </div>
                    <div>
                      <p className="text-[11px] font-black uppercase text-[#1F2937]">{resena.nombrePasajero || "Usuario"}</p>
                ...
                {resena.comentario && (
                  <p className="text-[11px] font-bold text-[#1F2937] italic bg-slate-50 p-4 rounded-[20px] border border-slate-100 mt-2">
                    "{resena.comentario}"
                  </p>
                )}
              </div>
