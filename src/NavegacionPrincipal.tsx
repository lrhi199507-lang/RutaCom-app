...
export default function NavegacionPrincipal({ user }: { user: any }) {
  ...
  const [modo, setModo] = useState('pasajero'); // NUEVO: Controla si es pasajero o chófer
  ...
  // NUEVO: Formulario para que el chófer publique
  ...
  // NUEVA: Función para publicar viaje como chófer
  const publicarViaje = async () => {
    if (!formViaje.destino || !formViaje.precio) {
      alert("Por favor indica destino y precio.");
      return;
    }
    try {
      ...
      setModo('pasajero'); // Regresa a vista pasajero para ver su propio viaje
      setFormViaje({ destino: "", precio: "", modeloAuto: "", placa: "", puestos: "4", detalles: "" });
  ...
  return (
    <div className="w-full max-w-md mx-auto h-screen bg-slate-50 relative overflow-hidden flex flex-col font-sans">
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="text-[10px] font-black uppercase text-slate-400 italic tracking-widest">RutaCom {modo === 'chofer' ? 'CHÓFER' : 'PRO'}</p>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        {vista === 'inicio' && modo === 'chofer' && (
          <div className="p-6 space-y-4 pb-32 text-left">
             ...
             <div className="bg-white p-6 rounded-[35px] border shadow-sm space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <select className="w-full bg-slate-50 p-4 rounded-2xl text-sm font-bold outline-none" value={formViaje.destino} onChange={(e) => setFormViaje({...formViaje, destino: e.target.value})}>
                ...
                <textarea className="w-full bg-slate-50 p-4 rounded-2xl text-sm font-bold outline-none h-24" value={formViaje.detalles} onChange={(e) => setFormViaje({...formViaje, detalles: e.target.value})} placeholder="Detalles extras (Hora de salida, punto de encuentro...)"/>
        ...
        {/* ... (Aquí siguen tus vistas de chat_conductor, chat_soporte y perfil igual que antes) */}
          ...
          <div className="h-full flex flex-col bg-white">
            <div className="p-6 pt-12 border-b flex items-center gap-4">
              ...
              <div className="w-11 h-11 bg-slate-900 rounded-full flex items-center justify-center text-white font-bold italic shadow-lg">
                {(viajeActivo?.conductor || "C")[0]}
              </div>
              <div className="text-left flex-1">
                <p className="text-sm font-black uppercase italic leading-none">{viajeActivo?.conductor || "Conductor"}</p>
            ...
            <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-slate-50/50 text-left">
              <div className="bg-white p-4 rounded-2xl rounded-bl-none shadow-sm max-w-[85%]">
                <p className="text-xs font-bold italic text-slate-700">Hola, soy tu conductor. Tu reserva está confirmada para {viajeActivo?.destino}.</p>
              </div>
              {mensajesConductor.map((c, i) => (
            ...
            <div className="p-4 border-t flex gap-3 items-center bg-white">
              <input type="text" value={inputConductor} onChange={(e) => setInputConductor(e.target.value)} placeholder="Escribe al conductor..." className="flex-1 bg-slate-100 p-4 rounded-2xl text-xs font-bold outline-none"/>
              <button onClick={enviarAConductor} className="w-14 h-14 bg-blue-600 rounded-2xl text-white flex items-center justify-center shadow-lg"><Send size={24}/></button>
            </div>
          </div>
        )}
        ...
        {vista === 'perfil' && (
           <div className="p-6 text-center space-y-6 pb-32">
              ...
              <div className="bg-slate-900 p-6 rounded-[35px] text-left shadow-xl relative overflow-hidden">
                 <div className="z-10 relative">
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase mt-2">Saldo retenido por viajes: ${Number(userData.saldoRetenido || 0).toFixed(2)}</p>
                 </div>
              ...
              <div className="bg-white p-8 rounded-[40px] border shadow-sm text-left">
                 <div className="space-y-6">
                    <div>
                       <p className="text-[9px] font-bold text-slate-400 uppercase mb-2 ml-1">Nombre de usuario</p>
                    ...
                    <div>
                       <p className="text-[9px] font-bold text-slate-400 uppercase mb-2 ml-1">Teléfono vinculado</p>
                 ...
                 {editando && (
                    <button onClick={async () => {
                      ...
                      setEditando(false);
                    }} className="w-full mt-4 bg-blue-600 text-white py-3 rounded-xl font-black uppercase text-[10px]">Guardar Cambios</button>
                 )}
              </div>
              <button onClick={() => auth.signOut()} className="w-full py-5 bg-red-50 text-red-500 font-black italic uppercase text-[11px] rounded-3xl border border-red-100 flex items-center justify-center gap-3"><LogOut size={16}/> Cerrar Sesión Segura</button>
           </div>
        )}
      </main>

      {/* Modal de Reserva Pasajero */}
      {viajeSeleccionado && modo === 'pasajero' && (
        <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-end">
          <div className="w-full bg-white rounded-t-[50px] p-8 space-y-6">
            ...
            <button onClick={() => manejarReserva(viajeSeleccionado)} className="w-full py-5 bg-blue-600 rounded-[22px] font-black text-[11px] uppercase text-white shadow-2xl flex items-center justify-center gap-3">
              Confirmar Reserva (${viajeSeleccionado.precio})
            </button>
          </div>
        </div>
      )}
      ...
      {!['chat_conductor', 'chat_soporte'].includes(vista) && (
        <nav className="absolute bottom-6 left-6 right-6 bg-slate-900/95 backdrop-blur-xl rounded-[35px] p-4 flex justify-around items-center z-40 border border-white/10 shadow-2xl">
          <button onClick={() => setVista('inicio')} className={`p-2 transition-all ${vista === 'inicio' ? 'text-blue-400 scale-125' : 'text-slate-50 opacity-50'}`}><Search size={26}/></button>
          <button onClick={() => setVista('chat_soporte')} className={`p-2 transition-all ${vista === 'chat_soporte' ? 'text-blue-400 scale-125' : 'text-slate-50 opacity-50'}`}><Headset size={26}/></button>
          <button onClick={() => setVista('perfil')} className={`p-2 transition-all ${vista === 'perfil' ? 'text-blue-400 scale-125' : 'text-slate-50 opacity-50'}`}><User size={26}/></button>
        </nav>
      )}
    </div>
  );
