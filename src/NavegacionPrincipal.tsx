...
export default function NavegacionPrincipal({ user }: { user: any }) {
  ...
  const publicarViaje = async () => {
    if (!formViaje.destino || !formViaje.precio || !formViaje.modeloAuto) {
      alert("⚠️ Completa los campos obligatorios (Auto, Destino y Precio)");
      return;
    }
    try {
      ...
      console.error(e);
      alert("❌ Error al publicar. Revisa tu conexión."); 
    }
  };
  ...
  return (
    <div className="w-full max-w-md mx-auto h-screen bg-slate-50 relative overflow-hidden flex flex-col">
        ...
        <header className="p-6 pt-12 bg-white border-b shrink-0 z-20">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 ${modo === 'chofer' ? 'bg-green-600' : 'bg-blue-600'} rounded-xl flex items-center justify-center text-white font-black italic text-xl transition-all shadow-lg`}>R</div>
        ...
        {vista === 'inicio' && modo === 'pasajero' && (
          <div className="p-6 space-y-6 pb-32">
             <div className="relative">
                ...
                {mostrarDestinos && (
                  <div className="absolute top-full left-0 right-0 bg-white border mt-2 rounded-3xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in duration-200">
                    {ESTADOS.map(estado => (
                      <button key={estado} className="w-full p-4 text-left text-sm font-bold border-b last:border-0 hover:bg-blue-50 transition-colors uppercase" 
        ...
        {vista === 'inicio' && modo === 'chofer' && (
          <div className="p-6 space-y-4 pb-32 text-left animate-in slide-in-from-right duration-300">
             ...
             <div className="bg-white p-6 rounded-[35px] border shadow-sm space-y-4">
                ...
                <textarea className="w-full bg-slate-50 p-4 rounded-2xl text-sm font-bold outline-none h-24 border" value={formViaje.detalles} onChange={(e) => setFormViaje({...formViaje, detalles: e.target.value})} placeholder="Notas: No mascotas, aire full..."/>
          ...
          <div className="h-full flex flex-col bg-white animate-in fade-in duration-300">
            <div className="p-6 pt-12 border-b flex items-center gap-4">
              ...
              <div className="text-left flex-1">
                ...
                <p className="text-[9px] text-green-500 font-black uppercase mt-1">● Chat de Viaje Activo</p>
              </div>
            </div>
            <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-slate-50/50 text-left">
              <div className="bg-white p-4 rounded-2xl rounded-bl-none shadow-sm max-w-[85%] border">
                <p className="text-xs font-bold italic text-slate-700">Hola, soy tu conductor. Ya recibí tu reserva para {viajeActivo?.destino}.</p>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => { if(confirm("¿Cancelar viaje?")) setVista('inicio'); }} className="py-3 bg-red-50 text-red-600 rounded-xl text-[10px] font-black uppercase border border-red-100">Cancelar</button>
                <button onClick={() => alert("Notificación enviada al chófer")} className="py-3 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase shadow-md">¡Ya llegué!</button>
              </div>
              {mensajesConductor.map((m, i) => (
                <div key={i} className={`p-4 rounded-2xl max-w-[85%] ${m.yo ? 'bg-blue-600 text-white ml-auto rounded-br-none' : 'bg-white border rounded-bl-none'}`}>
                  <p className="text-xs font-black italic">{m.texto}</p>
                </div>
              ))}
            </div>
            <div className="p-4 border-t flex gap-3 bg-white">
              <input type="text" value={inputConductor} onChange={(e) => setInputConductor(e.target.value)} placeholder="Escribe un mensaje..." className="flex-1 bg-slate-100 p-4 rounded-2xl text-xs font-bold outline-none"/>
        ...
        {vista === 'chat_soporte' && (
          <div className="h-full flex flex-col bg-white">
            <div className="p-6 pt-12 border-b flex items-center gap-4 bg-slate-900 text-white">
              ...
              <div className="text-left">
                <p className="text-sm font-black uppercase italic">Centro de Ayuda</p>
                <p className="text-[9px] text-blue-400 font-black uppercase">Soporte 24/7 Activo</p>
              </div>
            </div>
            <div className="flex-1 p-6 space-y-4 overflow-y-auto bg-slate-50/50">
              <div className="bg-white p-4 rounded-2xl rounded-bl-none border text-left max-w-[85%]">
                 <p className="text-xs font-black italic text-slate-700">Hola {userData.nombre}, ¿en qué podemos ayudarte con tu viaje o tu saldo?</p>
              </div>
            ...
            <div className="p-4 border-t flex gap-3 bg-white">
              <input type="text" value={inputSoporte} onChange={(e) => setInputSoporte(e.target.value)} placeholder="Describe tu problema..." className="flex-1 bg-slate-100 p-4 rounded-2xl text-xs font-bold outline-none"/>
      ...
      {/* MODAL DETALLES VIAJE (CORREGIDO EXTRAS) */}
      {viajeSeleccionado && (
