              ...
              {historialChats.map(c => {
                ...
                return (
                  <div key={c.chatId} onClick={() => abrirChat(c.idViaje, c.idOtro, c.nombreOtro)} className="bg-white p-4 rounded-3xl border shadow-sm flex items-center gap-4 cursor-pointer hover:border-blue-200 transition-all group relative overflow-hidden">
                    <div className={`absolute top-0 right-0 px-3 py-1 rounded-bl-xl text-[7px] font-black uppercase italic ${soyChofer ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}>Chat como {soyChofer ? 'Chofer' : 'Pasajero'}</div>
                    <div className="w-12 h-12 bg-blue-50 rounded-full flex items-center justify-center group-hover:bg-blue-600 transition-colors shrink-0"><User size={20} className="text-blue-500 group-hover:text-white"/></div>
              ...
              {pestañaActiva === "perfil" ? (
                <div className="bg-white p-8 rounded-[40px] shadow-sm border flex flex-col items-center">
                  <div className="w-28 h-28 bg-slate-100 rounded-full flex items-center justify-center border-4 border-white shadow-xl overflow-hidden">
                    {userData?.fotoPerfil ? <img src={userData.fotoPerfil} className="w-full h-full object-cover"/> : <User size={56} className="text-slate-400" />}
                  </div>
