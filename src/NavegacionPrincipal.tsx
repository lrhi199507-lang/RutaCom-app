...
import {
  ...
  Settings, Trash2, MessageCircle, CreditCard, Users, Info, ChevronLeft, MapPin
} from "lucide-react";
...
export default function NavegacionPrincipal({ user }: { user: any }) {
  ...
  // Estados para Chat con Chofer
  const [mensajesPrivados, setMensajesPrivados] = useState<any[]>([]);
  ...
  // Listener para saber si ya pedimos este viaje en TIEMPO REAL
  useEffect(() => {
    if (!viajeSeleccionado || !user) return;
    const q = query(
      collection(db, "Solicitudes"), 
      where("idViaje", "==", viajeSeleccionado.id),
      where("idPasajero", "==", user.uid)
    );
    const unsubSol = onSnapshot(q, (snap) => {
      if (!snap.empty) setSolicitudEnviada(snap.docs[0].id);
      else setSolicitudEnviada(null);
    });
    return () => unsubSol();
  }, [viajeSeleccionado, user]);

  // Listener para los mensajes del chat con el chofer
  useEffect(() => {
    if (vista !== "chat_viaje" || !viajeSeleccionado) return;
    const chatId = `${viajeSeleccionado.id}_${user.uid}`;
    const q = query(collection(db, "MensajesChat"), where("chatId", "==", chatId), orderBy("fecha", "asc"));
    const unsubChat = onSnapshot(q, (snap) => {
      setMensajesPrivados(snap.docs.map(d => d.data()));
    });
    return () => unsubChat();
  }, [vista, viajeSeleccionado, user]);
  ...
  const cancelarSolicitud = async () => {
    ...
    if (confirm("¿Seguro que quieres cancelar la cola?")) {
      try {
  ...
  const enviarMensajeChofer = async () => {
    if (!nuevoMensajePrivado.trim() || !viajeSeleccionado) return;
    try {
      await addDoc(collection(db, "MensajesChat"), {
        chatId: `${viajeSeleccionado.id}_${user.uid}`,
        texto: nuevoMensajePrivado,
        remitenteId: user.uid,
        fecha: serverTimestamp()
      });
      setNuevoMensajePrivado("");
  ...
  const guardarDatosPerfil = async () => {
    try {
      ...
      setEditandoPerfil(false); alert("✅ Perfil actualizado.");
    } catch (e) { alert("Error."); }
  };
  ...
  const enviarMensajeSoporte = () => {
    ...
    setMensajeSoporte("");
    setTimeout(() => setChatSoporte(p => [...p, { texto: "Un asesor responderá pronto.", mio: false }]), 1000);
  };
  ...
  return (
    <div className="w-full max-w-md mx-auto h-screen bg-slate-50 flex flex-col relative overflow-hidden font-sans">
      ...
      <main className="flex-1 overflow-y-auto p-5 pb-32">
        {/* VISTA DE CHAT PRIVADO CON EL CHOFER */}
        {vista === "chat_viaje" ? (
          <div className="flex flex-col h-full animate-in slide-in-from-bottom duration-300">
             <button onClick={() => setVista("inicio")} className="flex items-center gap-2 text-slate-400 font-black uppercase text-[10px] italic mb-4">
                <ChevronLeft size={16}/> Volver al Viaje
             </button>
             <div className="flex-1 flex flex-col bg-white rounded-[40px] border shadow-xl overflow-hidden">
                <div className="bg-slate-900 p-4 text-white flex items-center gap-3">
                   <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center font-black italic">!</div>
                   <div><p className="text-[8px] font-black uppercase opacity-60">Chófer</p><p className="text-xs font-black uppercase italic">{viajeSeleccionado?.conductor}</p></div>
                </div>
                <div className="flex-1 p-5 overflow-y-auto space-y-3 bg-slate-50">
                   {mensajesPrivados.map((m, i) => (
                     <div key={i} className={`p-3 rounded-2xl max-w-[80%] text-[11px] font-bold ${m.remitenteId === user.uid ? 'bg-blue-600 text-white self-end ml-auto' : 'bg-white border text-slate-700'}`}>
                        {m.texto}
                     </div>
                   ))}
                </div>
                <div className="p-4 bg-white border-t flex gap-2">
                   <input type="text" value={nuevoMensajePrivado} onChange={(e)=>setNuevoMensajePrivado(e.target.value)} className="flex-1 bg-slate-100 p-4 rounded-2xl text-xs font-bold outline-none border" placeholder="Escribe al chófer..." />
                   <button onClick={enviarMensajeChofer} className="bg-blue-600 w-12 h-12 rounded-2xl text-white flex items-center justify-center shadow-lg"><Send size={18}/></button>
                </div>
             </div>
          </div>
        ) : viajeSeleccionado ? (
               ...
               <div className="p-8 space-y-6">
                  ...
                  {viajeSeleccionado.extras && (
                    <div className="bg-slate-50 p-5 rounded-3xl border border-dashed border-slate-200">
                       <p className="text-[9px] font-black text-blue-600 uppercase mb-2 italic">Notas:</p>
                       <p className="text-xs font-bold text-slate-600 italic">"{viajeSeleccionado.extras}"</p>
                    </div>
                  )}
                  <div className="flex flex-col gap-3 pt-4">
                     <div className="flex gap-2">
                        <button onClick={() => setVista("chat_viaje")} className="flex-1 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase italic text-xs flex items-center justify-center gap-2 shadow-lg">
                        ...
                        {solicitudEnviada ? (
                          ...
                          <button onClick={enviarSolicitud} className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase italic text-xs shadow-lg">
                             Pedir Cola
                          </button>
                        )}
                     </div>
          ...
          vista === "inicio" && (
            <div className="space-y-6">
              <button onClick={() => setModo(modo === "pasajero" ? "chofer" : "pasajero")} className="w-full py-4 rounded-2xl text-[10px] font-black uppercase border-2 border-blue-600 text-blue-600 bg-white">
                MODO: {modo === "pasajero" ? "PASAJERO" : "CHÓFER"}
              </button>

              {modo === "chofer" ? (
                <div className="space-y-6">
                  <div className="bg-white p-6 rounded-[35px] border shadow-xl space-y-3">
                    <h3 className="text-xs font-black uppercase text-blue-600 italic flex items-center gap-2"><Navigation size={16}/> Publicar Ruta</h3>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <input type="number" placeholder="Puestos" className="bg-slate-50 p-4 rounded-xl border text-xs font-bold" value={form.puestos} onChange={(e)=>setForm({...form, puestos: e.target.value})} />
                  ...
                  {viajes.filter(v => v.idCreador === user.uid).map(v => (
                    <div key={v.id} className="bg-white p-4 rounded-3xl border flex justify-between items-center shadow-sm">
                      <div><p className="text-[10px] font-black uppercase text-slate-800 italic">{v.cO} → {v.cD}</p><p className="text-[10px] text-blue-600 font-bold italic">${v.precio}</p></div>
                ...
                <div className="space-y-4">
                  <div className="bg-white p-5 rounded-[30px] border space-y-3">
                  ...
                  {viajesFiltrados.map(v => (
                    <div key={v.id} className="bg-white p-5 rounded-[35px] border flex flex-col shadow-md space-y-3">
                      <div className="flex justify-between items-start">
                        <div className="flex-1"><div className="flex items-center gap-1 mb-1">{v.verificado && <ShieldCheck size={12} className="text-blue-500" />}<p className="text-[9px] font-black text-slate-400 uppercase italic">{v.conductor}</p></div><p className="font-black uppercase text-xs text-slate-800 italic">{v.cO} → {v.cD}</p></div>
        ...
        {/* PERFIL Y SOPORTE (NO SE TOCAN, QUEDAN IGUAL) */}
        {vista === "perfil" && (
           <div className="space-y-4">
              <div className="bg-white p-6 rounded-[35px] shadow-sm border flex flex-col items-center relative">
                 ...
                 <div className="flex items-center gap-2 mt-2 bg-slate-50 px-4 py-1.5 rounded-full border">
                    ...
                    <p className="text-[10px] font-black text-slate-600 uppercase italic">CI: {userData.cedula || "NO REGISTRADA"}</p>
                 </div>
              </div>
              {configOpen && (
