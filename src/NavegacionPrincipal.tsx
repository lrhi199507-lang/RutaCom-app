...
export default function NavegacionPrincipal({ user }: { user: any }) {
  ...
  const [solicitudEnviada, setSolicitudEnviada] = useState<string | null>(null); // ID de la solicitud si existe
  ...
  // Chequear si el pasajero ya pidió este viaje cuando abre el detalle
  useEffect(() => {
    if (viajeSeleccionado && user) {
      const q = query(
  ...
  return (
    <div className="w-full max-w-md mx-auto h-screen bg-slate-50 flex flex-col relative overflow-hidden font-sans">
      ...
      <main className="flex-1 overflow-y-auto p-5 pb-32">
               ...
               <div className="p-8 space-y-6">
                  ...
                  {viajeSeleccionado.extras && (
                    <div className="bg-slate-50 p-5 rounded-3xl border border-dashed border-slate-200">
                       <p className="text-[9px] font-black text-blue-600 uppercase mb-2 italic">Información Extra:</p>
                       <p className="text-xs font-bold text-slate-600 italic">"{viajeSeleccionado.extras}"</p>
                    </div>
                  )}

                  {/* BOTONES DE ACCIÓN: CHAT Y PEDIR COLA */}
                  <div className="flex flex-col gap-3 pt-4">
                     <div className="flex gap-2">
                        <button className="flex-1 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase italic text-xs flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all">
                           <MessageCircle size={18}/> Chat con Chófer
                        </button>
                        {solicitudEnviada ? (
                          <button onClick={cancelarSolicitud} className="flex-1 py-4 bg-red-500 text-white rounded-2xl font-black uppercase italic text-xs shadow-lg active:scale-95 transition-all">
                             Cancelar Cola
                          </button>
                        ) : (
                          <button onClick={enviarSolicitud} className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase italic text-xs shadow-lg shadow-blue-200 active:scale-95 transition-all">
                             Pedir Cola
                          </button>
                        )}
                     </div>
                     <button onClick={() => setViajeSeleccionado(null)} className="w-full py-3 text-slate-400 font-black uppercase italic text-[9px] tracking-widest">Cerrar Detalle</button>
                  </div>
               </div>
            </div>
          </div>
              ...
              {modo === "chofer" ? (
                <div className="space-y-6">
                  <div className="bg-white p-6 rounded-[35px] border shadow-xl space-y-3">
                    ...
                    <input type="text" placeholder="Extras (Aire, Maletas, etc...)" className="w-full bg-slate-50 p-4 rounded-xl border text-xs font-bold outline-none" value={form.extras} onChange={(e)=>setForm({...form, extras: e.target.value})} />
                  ...
                  {viajesFiltrados.map(v => (
                    <div key={v.id} className="bg-white p-5 rounded-[35px] border flex flex-col shadow-md space-y-3">
                      ...
                      <div className="flex items-center justify-between border-t pt-3">
                        ...
                        <button onClick={() => setViajeSeleccionado(v)} className="text-[9px] bg-slate-900 text-white px-8 py-2.5 rounded-full font-black uppercase italic shadow-lg active:scale-95 transition-all">Ver Detalle</button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {/* VISTAS DE PERFIL Y SOPORTE SE MANTIENEN IGUAL */}
        {vista === "perfil" && (
           <div className="space-y-4">
