...
export default function NavegacionPrincipal({ user }: { user: any }) {
  ...
  const publicarViaje = async () => {
    if (!formViaje.destino || !formViaje.precio || !formViaje.modeloAuto) {
      return alert(
        "⚠️ Por favor completa todos los campos del vehículo y destino",
      );
    }
    try {
  ...
  const manejarReserva = async (viaje: any) => {
    if (userData.saldo < viaje.precio)
      return alert("⚠️ Saldo insuficiente en tu billetera");
    try {
        ...
        <header className="p-6 pt-12 bg-white border-b shrink-0 z-20 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-3 text-left">
              <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white font-black shadow-lg italic">
                R
              </div>
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">
                  RutaCom {modo === "pasajero" ? "Pasajero" : "Chófer"}
                </p>
            ...
            <div className="bg-slate-900 text-white px-4 py-2 rounded-2xl flex items-center gap-2 border border-slate-700">
              ...
              <span className="text-[11px] font-black">
                ${Number(userData.saldo).toFixed(2)}
              </span>
            </div>
          </div>
          <button
            onClick={() => setModo(modo === "pasajero" ? "chofer" : "pasajero")}
            className="w-full py-2.5 bg-blue-50 text-blue-600 rounded-xl text-[10px] font-black uppercase border border-blue-100 hover:bg-blue-100 transition-colors"
          >
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        {vista === "inicio" && modo === "pasajero" && (
          <div className="p-6 space-y-4 pb-32">
            <button
              onClick={() => setMostrarDestinos(!mostrarDestinos)}
              className="w-full bg-white p-5 rounded-[25px] border flex items-center gap-4 shadow-sm hover:border-blue-300 transition-all"
            >
              ...
              <div className="bg-white border rounded-3xl shadow-xl overflow-hidden mb-4 animate-in fade-in slide-in-from-top-2">
                {ESTADOS.map((e) => (
                  <button
                    ...
                    className="w-full p-4 text-left font-bold border-b text-sm uppercase hover:bg-blue-50 text-slate-600"
                  >
                    {e}
                  </button>
                ))}
            ...
            {viajesReales
              .filter((v) => !busqueda || v.destino === busqueda)
              .map((v) => (
                <div
                  key={v.id}
                  onClick={() => setViajeSeleccionado(v)}
                  className="bg-white p-5 rounded-[30px] border flex justify-between items-center shadow-sm active:scale-95 transition-all cursor-pointer hover:shadow-md"
                >
                  <div className="flex gap-3 text-left">
                    ...
                    <div>
                      ...
                      <p className="font-black uppercase text-sm text-slate-800">
                        {v.conductor || "Chófer Profesional"}
                      </p>
                      <p className="text-[9px] text-slate-400 font-bold uppercase">
                        {v.vehiculo || "Vehículo verificado"}
                      </p>
                    </div>
                  </div>
        ...
        {vista === "inicio" && modo === "chofer" && (
          <div className="p-6 space-y-4 text-left pb-32">
            <h2 className="text-lg font-black uppercase italic flex items-center gap-2 text-slate-800">
              <PlusCircle size={20} className="text-green-600" /> Publicar Mi
              Ruta
            </h2>
            <div className="bg-white p-6 rounded-[35px] border space-y-4 shadow-sm">
              <input
                className="w-full bg-slate-50 p-4 rounded-2xl border border-slate-100 font-bold text-sm focus:border-blue-400 outline-none"
                placeholder="Modelo del Auto (Ej: Toyota Corolla)"
                value={formViaje.modeloAuto}
                onChange={(e) =>
                  setFormViaje({ ...formViaje, modeloAuto: e.target.value })
                }
              />
              <div className="grid grid-cols-2 gap-3">
                <select
                  className="bg-slate-50 p-4 rounded-2xl border border-slate-100 font-bold text-sm outline-none"
                  value={formViaje.destino}
                  onChange={(e) =>
                    setFormViaje({ ...formViaje, destino: e.target.value })
                  }
                >
                ...
                <input
                  ...
                  className="bg-slate-50 p-4 rounded-2xl border border-slate-100 font-bold text-sm outline-none"
                  placeholder="Precio $"
                  value={formViaje.precio}
                  onChange={(e) =>
                    setFormViaje({ ...formViaje, precio: e.target.value })
                  }
                />
              </div>
              <textarea
                className="w-full bg-slate-50 p-4 rounded-2xl border border-slate-100 font-bold text-sm h-24 outline-none resize-none"
                placeholder="Detalles (Ej: Aire Full, Maletero amplio...)"
                value={formViaje.detalles}
                onChange={(e) =>
                  setFormViaje({ ...formViaje, detalles: e.target.value })
                }
              />
              <button
                onClick={publicarViaje}
                className="w-full py-4 bg-green-600 text-white rounded-[20px] font-black uppercase italic shadow-lg active:scale-95 transition-transform hover:bg-green-700"
              >
                Publicar Ahora
              </button>
            </div>
          ...
          <div className="p-6 space-y-6 text-center pb-32">
            <div className="w-24 h-24 bg-blue-600 rounded-[35px] mx-auto flex items-center justify-center text-white shadow-xl relative border-4 border-white">
              ...
              <button
                ...
                className={`absolute -bottom-2 -right-2 p-2 rounded-xl text-white border-2 border-white transition-colors ${editando ? "bg-green-500" : "bg-slate-900"}`}
              >
                <Edit2 size={14} />
              </button>
            </div>
            <div className="text-left space-y-4">
              <div className="bg-slate-900 p-6 rounded-[30px] shadow-xl border border-slate-800">
                <p className="text-blue-400 text-[10px] font-black uppercase italic">
                  Billetera Digital
                </p>
              ...
              <div className="bg-white p-6 rounded-[35px] border space-y-4 shadow-sm">
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase ml-2 mb-1">
                    Nombre Completo
                  </p>
                  <input
                    disabled={!editando}
                    className={`w-full p-4 rounded-2xl font-bold uppercase text-sm border transition-all ${editando ? "bg-white border-blue-200" : "bg-slate-50 border-transparent text-slate-500"}`}
                ...
                <div>
                  ...
                  <input
                    disabled={!editando}
                    className={`w-full p-4 rounded-2xl font-bold text-sm border transition-all ${editando ? "bg-white border-blue-200" : "bg-slate-50 border-transparent text-slate-500"}`}
                ...
                {editando && (
                  <button
                    ...
                    className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black uppercase italic text-xs shadow-lg animate-in fade-in zoom-in-95"
                  >
                    Guardar Cambios
                  </button>
                )}
            ...
            <button
              ...
              className="w-full py-4 bg-red-50 text-red-500 font-black uppercase rounded-2xl border border-red-100 flex items-center justify-center gap-2 italic text-xs hover:bg-red-100 transition-colors"
            >
        ...
        {(vista === "chat_conductor" || vista === "chat_soporte") && (
          <div className="absolute inset-0 z-50 flex flex-col bg-white">
            <div
              className={`p-6 pt-12 border-b flex items-center gap-4 shrink-0 ${vista === "chat_soporte" ? "bg-slate-900 text-white" : "bg-white text-slate-900"}`}
            >
              <button
            ...
            <div className="flex-1 p-6 space-y-4 overflow-y-auto bg-slate-50">
              <div className="bg-white p-4 rounded-2xl border border-slate-100 text-left shadow-sm max-w-[85%]">
                <p className="text-xs font-bold italic text-slate-600">
                  {vista === "chat_soporte"
                    ? "Hola Luis, ¿en qué podemos ayudarte hoy con tu viaje?"
                    : `Hola ${userData.nombre}, estoy atento a tu reserva para ${viajeActivo?.destino}.`}
                </p>
              </div>
              {(vista === "chat_conductor"
                ...
                <div
                  key={i}
                  className={`p-4 rounded-2xl max-w-[80%] text-left shadow-sm ${m.yo ? "bg-blue-600 text-white ml-auto rounded-tr-none" : "bg-white border rounded-tl-none"}`}
                >
            ...
            <div className="p-4 border-t flex gap-2 bg-white pb-8">
              <input
                className="flex-1 bg-slate-100 p-4 rounded-2xl text-sm outline-none font-bold placeholder:text-slate-400"
                placeholder="Escribe un mensaje..."
                value={
                  vista === "chat_conductor" ? inputConductor : inputSoporte
                }
                onChange={(e) =>
                  vista === "chat_conductor"
                    ? setInputConductor(e.target.value)
                    : setInputSoporte(e.target.value)
                }
                onKeyPress={(e) => {
                  if (e.key === "Enter") {
                    const text =
                      vista === "chat_conductor"
                        ...
                        : inputSoporte;
                    if (!text) return;
                    if (vista === "chat_conductor") {
                      setMensajesConductor([
                        ...mensajesConductor,
                        { texto: text, yo: true },
                      ]);
                      setInputConductor("");
              ...
              <button
                ...
                className="p-4 bg-blue-600 text-white rounded-2xl shadow-lg active:scale-90 transition-transform"
              >
                <Send size={20} />
              </button>
            </div>
          ...
          <div className="w-full bg-white rounded-t-[50px] p-8 space-y-6 shadow-2xl animate-in slide-in-from-bottom">
            <div className="flex justify-between items-start">
              <div className="flex gap-4 text-left">
                <div className="w-14 h-14 bg-blue-600 rounded-[20px] flex items-center jus
