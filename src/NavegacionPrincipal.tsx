...
export default function NavegacionPrincipal({ user }) {
  ...
  // /* NUEVO: FUNCIÓN PARA PEDIR LA COLA DIRECTAMENTE DESDE EL CHAT */
  const pedirColaDesdeChat = async (viaje) => {
    try {
      const docRef = await addDoc(collection(db, "Solicitudes"), {
        idViaje: viaje.id,
        idPasajero: user.uid,
        nombrePasajero: userData.nombre || "Pasajero",
        idChofer: viaje.idCreador,
        nombreChofer: viaje.conductor, 
        ruta: `${viaje.cO} → ${viaje.cD}`,
        estado: "pendiente",
        fechaSolicitud: serverTimestamp()
      });
      if (viajeSeleccionado && viajeSeleccionado.id === viaje.id) {
         setSolicitudEnviada(docRef.id);
      }
      alert("✅ Solicitud enviada al chofer.");
    } catch (e) { alert("Error al solicitar desde el chat."); }
  };

  // /* NUEVO: FUNCIÓN PARA QUE EL CHOFER ACEPTE LA SOLICITUD */
  const aceptarSolicitud = async (idSolicitud) => {
    try {
      await updateDoc(doc(db, "Solicitudes", idSolicitud), { estado: "confirmado" });
      alert("✅ ¡Has confirmado este viaje!");
    } catch (e) { alert("Error al confirmar el viaje."); }
  };
  ...
  // /* NUEVO: VARIABLES PARA RENDERIZAR DETALLES Y BOTONES EN EL CHAT */
  const viajeDelChat = chatActivo ? viajes.find(v => v.id === chatActivo.idViaje) : null;
  const solicitudActual = chatActivo ? (
    modo === "pasajero" 
      ? misSolicitudes.find(s => s.idViaje === chatActivo.idViaje && s.idChofer === chatActivo.idOtro)
      : solicitudesRecibidas.find(s => s.idViaje === chatActivo.idViaje && s.idPasajero === chatActivo.idOtro)
  ) : null;
        ...
        {vista === "chat_privado" ? (
          <div className="flex flex-col h-full space-y-4 animate-in slide-in-from-right duration-300">
            ...
            <div className="flex-1 bg-white rounded-[40px] border shadow-xl flex flex-col overflow-hidden">
               ...
               {/* NUEVO: DETALLES DEL VIAJE DENTRO DEL CHAT Y BOTONES DE ACCIÓN */}
               {viajeDelChat && (
                  <div className="bg-blue-50 border-b border-blue-100 p-4 flex flex-col gap-2 shadow-inner shrink-0">
                     <div className="flex justify-between items-center">
                        <p className="text-[10px] font-black uppercase text-blue-800 italic">Detalles de la Cola</p>
                        <p className="font-black text-blue-600 italic">${viajeDelChat.precio}</p>
                     </div>
                     <p className="text-xs font-bold text-slate-700 flex items-center gap-1"><MapPin size={12} className="text-blue-500"/> {viajeDelChat.cO} → {viajeDelChat.cD}</p>
                     
                     {/* BOTONES SEGÚN EL MODO Y ESTADO DE LA SOLICITUD */}
                     {modo === "pasajero" && (
                        <div className="flex gap-2 mt-2">
                           {!solicitudActual ? (
                              <button onClick={() => pedirColaDesdeChat(viajeDelChat)} className="flex-1 py-2 bg-blue-600 text-white rounded-xl font-black uppercase text-[10px] shadow-md flex justify-center items-center gap-1"><Car size={14}/> Pedir Cola</button>
                           ) : solicitudActual.estado === "pendiente" ? (
                              <button onClick={() => setModalCancelacion({ visible: true, idSolicitud: solicitudActual.id })} className="flex-1 py-2 bg-red-500 text-white rounded-xl font-black uppercase text-[10px] shadow-md flex justify-center items-center gap-1"><AlertTriangle size={14}/> Cancelar Petición</button>
                           ) : solicitudActual.estado === "confirmado" ? (
                              <div className="flex-1 py-2 bg-green-500 text-white rounded-xl font-black uppercase text-[10px] text-center shadow-md flex justify-center items-center gap-1"><CheckCircle size={14}/> Viaje Confirmado</div>
                           )}
                        </div>
                     )}
                     ...
                     {modo === "chofer" && (
                        <div className="flex gap-2 mt-2">
                           {!solicitudActual ? (
                              <div className="flex-1 py-2 bg-slate-200 text-slate-500 rounded-xl font-black uppercase text-[9px] text-center shadow-inner">Esperando petición del pasajero...</div>
                           ) : solicitudActual.estado === "pendiente" ? (
                              <>
                                <button onClick={() => aceptarSolicitud(solicitudActual.id)} className="flex-1 py-2 bg-green-500 text-white rounded-xl font-black uppercase text-[10px] shadow-md flex justify-center items-center gap-1"><CheckCircle size={14}/> Confirmar Viaje</button>
                                <button onClick={() => setModalCancelacion({ visible: true, idSolicitud: solicitudActual.id })} className="flex-1 py-2 bg-red-500 text-white rounded-xl font-black uppercase text-[10px] shadow-md flex justify-center items-center gap-1"><AlertTriangle size={14}/> Rechazar</button>
                              </>
                           ) : solicitudActual.estado === "confirmado" ? (
                              <div className="flex-1 py-2 bg-green-500 text-white rounded-xl font-black uppercase text-[10px] text-center shadow-md flex justify-center items-center gap-1"><CheckCircle size={14}/> Viaje Confirmado</div>
                           )}
