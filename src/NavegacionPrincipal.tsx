...
import {
  ...
  Star, Settings, ChevronRight, Edit, Trash2, MessageCircle
} from "lucide-react";
...
// --- COMPONENTE PRINCIPAL (EXPORT DEFAULT OBLIGATORIO) ---
export default function NavegacionPrincipal({ user }: { user: any }) {
  ...
  // Buscador Pasajero (LOS 4 APARTADOS)
  ...
  const publicarRuta = async () => {
    if (!form.cO || !form.cD || !form.precio) return alert("Por favor, llena todos los campos de origen y destino.");
    try {
      // CORRECCIÓN CRÍTICA: Aseguramos datos planos para evitar error de base de datos
      await addDoc(collection(db, "Viajes"), {
        eO: form.eO, cO: form.cO, eD: form.eD, cD: form.cD,
        precio: Number(form.precio), puestos: form.puestos,
  ...
  return (
    <div className="w-full max-w-md mx-auto h-screen bg-slate-50 flex flex-col relative overflow-hidden">
          ...
          <div className="space-y-6">
            ...
            {modo === "chofer" ? (
              ...
              <div className="space-y-4">
                {/* BUSCADOR 4 SELECTORES (PASAJERO) */}
        ...
        {vista === "perfil" && (
          <div className="space-y-4 animate-in fade-in">
             <div className="bg-white p-6 rounded-[35px] shadow-sm border flex flex-col items-center relative">
                <button onClick={()=>setConfigOpen(!configOpen)} className="absolute top-6 right-6 p-2 bg-slate-50 rounded-xl text-blue-600"><Settings size={20}/></button>
             ...
             {configOpen && (
               <div className="bg-white p-6 rounded-[35px] border-2 border-blue-600 space-y-4 shadow-2xl">
                  ...
                  <div className="space-y-2">
                    ...
                    <button onClick={()=>alert("Cargar Rostro")} className="w-full p-4 bg-slate-50 rounded-2xl flex justify-between items-center text-[10px] font-black uppercase italic"><span>Verificación de Rostro</span><User size={16}/></button>
      ...
      <nav className="p-6 bg-white border-t flex justify-around items-center pb-10 fixed bottom-0 w-full max-w-md shadow-2xl">
        <button onClick={() => {setVista("inicio"); setConfigOpen(false);}} className={`flex flex-col items-center gap-1 ${vista === "inicio" ? "text-blue-600" : "text-slate-300"}`}>
          <Car size={24} /><span className="text-[9px] font-black uppercase italic">Viajes</span>
        </button>
        <button onClick={() => alert("Soporte Técnico")} className="flex flex-col items-center gap-1 text-slate-300">
          <MessageCircle size={24} /><span className="text-[9px] font-black uppercase italic">Soporte</span>
        </button>
        <button onClick={() => {setVista("perfil"); setConfigOpen(false);}} className={`flex flex-col items-center gap-1 ${vista === "perfil" ? "text-blue-600" : "text-slate-300"}`}>
