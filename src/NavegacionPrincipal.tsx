...
import {
  ...
  Headset, PlusCircle, ShieldCheck, Camera, Mail, Lock, CheckCircle, MapPin
} from "lucide-react";
...
export default function NavegacionPrincipal({ user }: { user: any }) {
  ...
  const [userData, setUserData] = useState<any>({ nombre: "", saldo: 0, telefono: "", calle: "", kycVerificado: false });
  ...
  // ESCUCHAR DATOS DE USUARIO (Incluye Calle y KYC)
  useEffect(() => {
    if (!user) return;
  ...
  const publicarViaje = async () => {
    if (!formViaje.destino || !formViaje.precio || !formViaje.modeloAuto) return alert("⚠️ Completa los campos");
    try {
  ...
  return (
    <div className="w-full max-w-md mx-auto h-screen bg-slate-50 relative overflow-hidden flex flex-col font-sans">
        ...
        <header className="p-6 pt-12 bg-white border-b shrink-0 z-20 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-3 text-left">
              <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white font-black shadow-lg italic text-lg">R</div>
              <div>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        {vista === "inicio" && modo === "pasajero" && (
          <div className="p-6 space-y-4 pb-32">
            <button onClick={() => setMostrarDestinos(!mostrarDestinos)} className="w-full bg-white p-5 rounded-[25px] border flex items-center gap-4 shadow-sm text-left">
            ...
            {mostrarDestinos && (
              <div className="bg-white border rounded-3xl shadow-xl overflow-hidden mb-4 animate-in slide-in-from-top-2">
                {ESTADOS.map((e) => (
                  <button key={e} onClick={() => { setBusqueda(e); setMostrarDestinos(false); }} className="w-full p-4 text-left font-bold border-b text-sm uppercase hover:bg-blue-50 text-slate-600 transition-colors">{e}</button>
                ))}
              </div>
            )}
            {viajesReales.filter((v) => !busqueda || v.destino === busqueda).map((v) => (
              <div key={v.id} onClick={() => setViajeSeleccionado(v)} className="bg-white p-5 rounded-[30px] border flex justify-between items-center shadow-sm cursor-pointer active:scale-95 transition-transform">
        ...
        {vista === "inicio" && modo === "chofer" && (
          <div className="p-6 space-y-4 text-left pb-32">
            ...
            <div className="bg-white p-6 rounded-[35px] border space-y-4 shadow-sm">
              ...
              <textarea className="w-full bg-slate-50 p-4 rounded-2xl border border-slate-100 font-bold text-sm h-24 outline-none resize-none" placeholder="Detalles del viaje..." value={formViaje.detalles} onChange={(e) => setFormViaje({ ...formViaje, detalles: e.target.value })} />
        ...
        {vista === "perfil" && (
          <div className="p-6 space-y-6 text-center pb-32">
            <div className="w-24 h-24 bg-blue-600 rounded-[35px] mx-auto flex items-center justify-center text-white shadow-xl relative border-4 border-white">
              <User size={40} />
              {userData.kycVerificado && <div className="absolute -bottom-1 -right-1 bg-blue-500 p-2 rounded-xl border-4 border-white"><CheckCircle size={16}/></div>}
              <button onClick={() => setEditando(!editando)} className={`absolute -top-2 -right-2 p-2 rounded-xl text-white border-2 border-white ${editando ? "bg-green-500" : "bg-slate-900"}`}><Edit2 size={14} /></button>
            </div>
            ...
            <div className="text-left space-y-4">
              ...
              <div className="bg-white p-6 rounded-[35px] border space-y-4 shadow-sm">
                ...
                <div className="space-y-1">
                  <p className="text-[9px] font-black text-slate-400 uppercase ml-2">Teléfono</p>
                ...
                <div className="space-y-1">
                  <p className="text-[9px] font-black text-blue-600 uppercase ml-2">Dirección / Calle</p>
                  <input disabled={!editando} className="w-full p-4 rounded-2xl font-bold text-sm border bg-blue-50/50 border-blue-100 placeholder:text-blue-200" placeholder="Ej: Calle C, Casa #10" value={formPerfil.calle} onChange={(e) => setFormPerfil({ ...formPerfil, calle: e.target.value })} />
                </div>
                
                {editando && (
                  <button onClick={async () => { await updateDoc(doc(db, "usuarios", user.uid), formPerfil); setEditando(false); alert("✅ Datos guardados"); }} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black uppercase italic shadow-lg">Guardar Cambios</button>
                )}

                {!userData.kycVerificado && (
                  <button onClick={() => setVerificandoKYC(true)} className="w-full py-3 bg-slate-100 text-slate-600 rounded-xl font-black uppercase text-[10px] flex items-center justify-center gap-2">
                    <ShieldCheck size={14} /> Verificar Identidad (KYC)
                  </button>
                )}
              </div>
            </div>
        ...
        {(vista === "chat_conductor" || vista === "chat_soporte") && (
          <div className="absolute inset-0 z-50 flex flex-col bg-white">
            ...
            {vista === "chat_conductor" && (
                <div className="px-4 py-2 bg-white flex flex-col gap-2 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
                  ...
                  {estadoViaje === 'confirmado' && (
                    <button onClick={() => { setEstadoViaje('finalizado'); alert("¡Viaje Finalizado!"); setVista('inicio'); }} className="w-full py-3 bg-green-600 text-white rounded-xl font-black uppercase italic text-[10px] shadow-lg">Ya llegué (Finalizar)</button>
                  )}
                </div>
            )}
            <div className="p-4 border-t flex gap-2 bg-white pb-8">
              <input className="flex-1 bg-slate-100 p-4 rounded-2xl text-sm outline-none font-bold placeholder:italic" placeholder="Escribe..." value={vista === "chat_conductor" ? inputConductor : inputSoporte} onChange={(e) => vista === "chat_conductor" ? setInputConductor(e.target.value) : setInputSoporte(e.target.value)} />
              <button onClick={() => {
      ...
      {/* MODAL KYC (IDENTIDAD) */}
      {verificandoKYC && (
        <div className="absolute inset-0 z-[100] bg-slate-900/70 backdrop-blur-md flex items-center justify-center p-6">
           <div className="bg-white w-full rounded-[40px] p-8 relative shadow-2xl animate-in zoom-in duration-300">
              ...
              <h3 className="font-black uppercase text-xl italic text-slate-800 mb-2">Verificar Identidad</h3>
              <p className="text-[10px] font-bold text-slate-400 uppercase mb-6 leading-relaxed">Sube fotos claras de tu documento para mayor seguridad.</p>
              
              <div className="space-y-3">
                 <div className="bg-slate-50 p-4 rounded-3xl border border-dashed border-slate-300 flex items-center justify-between">
                   <div className="flex items-center gap-3"><Camera size={18} className="text-blue-600"/><p className="text-[10px] font-black uppercase">Cédula Frontal</p></div>
                   <button className="px-4 py-2 bg-slate-900 text-white text-[9px] font-black rounded-lg">SUBIR</button>
                 </div>
                 <div className="bg-slate-50 p-4 rounded-3xl border border-dashed border-slate-300 flex items-center justify-between">
                   <div className="flex items-center gap-3"><Camera size={18} className="text-blue-600"/><p className="text-[10px] font-black uppercase">Cédula Trasera</p></div>
                   <button className="px-4 py-2 bg-slate-900 text-white text-[9px] font-black rounded-lg">SUBIR</button>
                 </div>
                 ...
                 }} className="w-full py-4 bg-blue-600 text-white font-black rounded-2xl uppercase text-[11px] italic mt-4 shadow-xl">Enviar Revisión</button>
              </div>
           </div>
        </div>
      )}
      ...
      {viajeSeleccionado && (
        <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-end animate-in fade-in duration-300">
          <div className="w-full bg-white rounded-t-[50px] p-8 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10">
      ...
      {/* NAVBAR */}
      <nav className="p-6 bg-white border-t flex justify-around items-center shrink-0 z-20 pb-10 shadow-[0_-5px_15px_rgba(0,0,0,0.02)]">
        ...
        <button onClick={() => setVista("chat_soporte")} className={`flex flex-col items-center gap-1 transition-colors ${vista === "chat_soporte" ? "text-blue-600" : "text-slate-400"}`}>
          <Headset size={24} /><span className="text-[9px] font-black uppercase tracking-tighter">Soporte</span>
        </button>
        <button onClick={() => setVista("perfil")} className={`flex flex-col items-center gap-1 transition-colors ${vista === "perfil" ? "text-blue-600" : "text-slate-400"}`}>
